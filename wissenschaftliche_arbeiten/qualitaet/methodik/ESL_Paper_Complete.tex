...
Behavioral Economics$^\dagger$ & 0.75 & \textbf{0.70} & [0.48, 0.91] & 0.15 & \textcolor{green!60!black}{Validated} (N=7) \\
...
Behavioral Econ & 0.75 & \textbf{0.70} & [0.48, 0.91] & 0.85 \\
Psychology & 0.72 & --- & --- & 0.82 \\
\bottomrule
\end{tabular}
\end{center}

\noindent {\small $^\dagger$ Empirically derived from N=60 validated claims (Appendices C--E). CI via SE = Gap$/2\sqrt{N}$}
...
Behavioral Economics & 0.75 & \textbf{0.70} & [0.55, 0.84] & 7 & T5 $\to$ T2 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{warningbox}
\textbf{Note on Behavioral Economics CI:} The wide confidence interval reflects limited sample size (N=7). The point estimate (0.70) is reliable, but the bounds should narrow with additional validated cases. Priority: expand BE validation dataset.
\end{warningbox}
