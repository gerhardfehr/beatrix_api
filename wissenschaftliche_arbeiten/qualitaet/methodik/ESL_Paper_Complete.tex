...
\begin{table}[h]
\centering
\caption{LLM Validation: Method Comparison}
\begin{tabular}{lccc}
\toprule
\textbf{Metric} & \textbf{Option A} & \textbf{Option B} & \textbf{Option C} \\
\midrule
Sample Size & 5 authors, 15 passages & 10 authors & 3 authors, 15 runs \\
MAD vs Manual & 0.090 & \textbf{0.027} & --- \\
Best Use Case & Detail analysis & \textbf{Screening} & Quality assurance \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Option A Results (Passage-Based):}
\begin{table}[h]
\centering
\begin{tabular}{lcccc}
\toprule
\textbf{Author} & \textbf{Tier} & \textbf{Manual K} & \textbf{LLM K} & \textbf{$\Delta$} \\
\midrule
Kahneman & 1 & 0.88 & 0.76 & $-0.12$ \\
Pinker & 1 & 0.82 & 0.94 & $+0.12$ \\
Harari & 3 & 0.55 & 0.58 & $+0.03$ \\
Gladwell & 3 & 0.42 & 0.53 & $+0.11$ \\
Taleb (core) & 1 & 0.85 & 0.83 & $-0.02$ \\
Taleb (polemic) & 3 & 0.35 & 0.21 & $-0.14$ \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Option B Results (Characterization):}
\begin{table}[h]
\centering
\begin{tabular}{lcccc}
\toprule
\textbf{Author} & \textbf{Tier} & \textbf{Manual K} & \textbf{LLM K} & \textbf{$\Delta$} \\
\midrule
Kahneman & 1 & 0.88 & 0.85 & $-0.03$ \\
Rosling & 1 & 0.87 & 0.88 & $+0.01$ \\
Piketty & 2 & 0.75 & 0.78 & $+0.03$ \\
Harari & 3 & 0.55 & 0.55 & $0.00$ \\
Gladwell & 3 & 0.42 & 0.45 & $+0.03$ \\
Taleb (core) & 1 & 0.85 & 0.85 & $0.00$ \\
Taleb (polemic) & 3 & 0.35 & 0.25 & $-0.10$ \\
Brown & 4 & 0.32 & 0.35 & $+0.03$ \\
Thaler & 1 & 0.84 & 0.82 & $-0.02$ \\
Peterson & 3 & 0.48 & 0.50 & $+0.02$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{resultbox}
\textbf{LLM Validation Results:}

\begin{enumerate}
    \item \textbf{Option B is most accurate}: MAD = 0.027 (vs 0.090 for Option A)
    \item \textbf{Tier correlates with consistency}: Tier 1 $\kappa_{MC} \approx 5\%$, Tier 3 $\kappa_{MC} \approx 12\%$
    \item \textbf{Bimodal detection works}: Taleb variance ($\sigma = 0.30$) correctly identified
    \item \textbf{Slight Tier 3 overestimation}: LLM tends to rate overclaimers slightly higher
    \item \textbf{Holistic $>$ Passage}: Author characterization outperforms passage analysis
\end{enumerate}
\end{resultbox}

\textbf{Recommendation}: Use Option B (Author-Characterization) for initial screening, Option A for disputed cases, Option C for quality assurance with $\kappa_{MC} < 10\%$ as stability threshold.
...
This analysis is classified as \textbf{T3/T4} (Mixed Methodology):
\begin{itemize}
    \item LLM cross-validation completed (Section~\ref{subsec:llm-validation})
    \item Option B shows MAD = 0.027 vs manual assessment
    \item Intra-LLM consistency measured: $\kappa_{MC}$ = 5--13\%
    \item Sample size (35 authors) is illustrative, not exhaustive
\end{itemize}

\textbf{Updated $E_{\max} = 0.60$} (improved from 0.45 via LLM validation)

\textbf{Remaining work}: Independent human raters, larger corpus, prospective validation.
\end{warningbox}

\begin{eslbox}
\textbf{Appendix Assessment:} This appendix presents literature analysis (B $\approx$ 0.70) with LLM-validated methodology (E $\approx$ 0.60). \\[0.3em]
\textbf{Calculation:} $K = 1 - |0.70 - 0.60| / 0.70 = 0.86$ \\
\textbf{Status:} \textcolor{green!60!black}{Stable}---LLM cross-validation supports framework validity
\end{eslbox}
