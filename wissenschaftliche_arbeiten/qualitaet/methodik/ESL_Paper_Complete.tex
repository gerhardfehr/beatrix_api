...
\textbf{ESL 2.2 (with N=95 empirical validation)} & \textbf{0.82} & \textbf{Stable} & \textcolor{green!60!black}{\textbf{Current}} \\
\bottomrule
\end{tabular}
\end{table}
...
The empirical validation (N=95, 100\% accuracy) fundamentally changes ESL's epistemic status:
\begin{itemize}
...
2: K-thresholds & 0.78 & 0.20 & \textbf{0.88} & \textbf{0.87} & N=95 validation \\
3: B-value calibration & 0.72 & 0.35 & 0.55 & 0.72 & Partial (linguistic) \\
\midrule
\textbf{Document (weighted)} & 0.76 & 0.47 & \textbf{0.76} & \textbf{0.82} & \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Evidence (E = 0.88)}: 95/95 = 100\% accuracy across Physics, Economics, Behavioral Economics, Narratives
    \item \textbf{K} = $1 - |0.78 - 0.88| / 0.78 = 1 - 0.13 = 0.87$
\end{itemize}

\textbf{Document-Level Assessment:}
\begin{itemize}
    \item \textbf{Weighted K} = $(0.91 \times 0.3) + (0.87 \times 0.5) + (0.72 \times 0.2) = 0.27 + 0.44 + 0.14 = 0.82$
...
\textit{``Confidence earned: K = 0.30 $\to$ K = 0.82 through systematic validation.''}
}}
\end{center}
...
\begin{tikzpicture}[scale=1.2]
    ...
    \node[below=5pt] at (8.5,0) {\textbf{K=0.82}};
    \node[below=18pt, green!60!black] at (8.5,0) {\footnotesize \textbf{Validated}};

    % Arrow showing journey
    \draw[thick, dashed, ->] (1,0.8) to[bend left=30] node[above, midway] {\footnotesize N=95} (8.5,0.8);
\end{tikzpicture}
