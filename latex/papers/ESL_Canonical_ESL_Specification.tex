...
\title{\textbf{Canonical ESL-Targeting Prompt Specification}\\
\Large $\pi_{\text{ESL}}$ --- Prompt Targeting Empirical Stability of Language}
...
\subsection{ESL: The Property Being Targeted}

\textbf{Empirical Stability of Language (ESL)} is the empirically observable property of assertive language whereby the strength of a linguistic claim ($K$) is proportionate to the strength of its supporting evidence ($M$):
\[
\text{ESL} \iff K \le M
\]

ESL is a \emph{descriptive property} to be measured, not a control mechanism. This prompt specification ($\pi_{\text{ESL}}$) is a \emph{prompt targeting ESL}---it aims to induce outputs that satisfy the ESL criterion.
