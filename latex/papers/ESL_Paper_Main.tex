...
The finding that activation competence is bounded---even under maximally explicit prompting---suggests a more radical architectural implication: epistemic control may need to be externalized from the language model entirely.

We propose a separation of concerns:
\begin{itemize}
    \item \textbf{Epistemic control} $\in$ Code: A formal theoretical model $\mathcal{T}$, executed by a code interpreter, determines which epistemic commitments are permitted.
    \item \textbf{Linguistic realization} $\in$ LLM: The language model serves solely as a constrained generator, producing text within boundaries set by $\mathcal{T}$.
\end{itemize}

Under this architecture, the theoretical model computes, for each candidate assertion $e_i$:
\[
...
Crucially, this yields:
\[
\alpha_{\text{system}} = 1 \quad \text{(for specified epistemic acts)}
\]
not because the model ``thinks better,'' but because it is no longer permitted to decide. Epistemic guarantees become \emph{computed} rather than \emph{generated}.

This approach satisfies strong constraints:
\begin{itemize}
...
\begin{quote}
\textit{Epistemic guarantees cannot be generated; they must be computed.}
\end{quote}
