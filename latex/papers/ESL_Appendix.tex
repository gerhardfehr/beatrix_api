...
\subsection{ESL Prompt ($\pi_{\text{ESL}}$)}
\label{app:esl-prompt}

The ESL prompt adds explicit calibration rules and anti-heuristics to the baseline.
...
The ESL prompt adds the following components not present in the baseline:
...
\begin{tabular}{lcc}
\toprule
\textbf{Component} & \textbf{Baseline} & \textbf{ESL} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Structural differences between Baseline and ESL prompts.}
